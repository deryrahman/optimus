...
func deployJob(deployTimeoutCtx context.Context,
	...
	for _, spec := range jobSpecs {
		...
		if err != nil {
			return errors.Wrapf(err, "[%s] failed to serialize: %s", namespaceName, spec.Name)
		}
	...
	if err != nil {
		...
		return errors.Wrapf(err, "[%s] deployement failed", namespaceName)
	}
	...
	for {
		...
		if resp.Ack {
			...
			if !resp.GetSuccess() {
				return errors.Errorf("[%s] unable to deploy: %s %s", namespaceName, resp.GetJobName(), resp.GetMessage())
			}
			ackCounter++
			if verbose {
	...
	if streamError != nil {
		if ackCounter == totalJobs {
			...
			return errors.Wrapf(streamError, "[%s] failed to receive success deployment ack", namespaceName)
		}
	}
...
func deployResource(deployTimeoutCtx context.Context,
	...
	for storeName, repoFS := range datastoreSpecFs {
		...
		if err != nil {
			return errors.Wrapf(err, "[%s] resourceSpecRepo.GetAll()", namespaceName)
		}
		...
		if err := stream.Send(&pb.DeployResourceSpecificationRequest{
			...
			return errors.Wrapf(err, "[%s] deployment failed", namespaceName)
		}
		rsp, err := stream.Recv()
		if err != nil {
			return errors.Wrapf(err, "[%s] receiving failed", namespaceName)
		}
		if rsp.Ack {
			if !rsp.Success {
			...
			if err := stream.CloseSend(); err != nil {
				return errors.Wrapf(err, "[%s] close connection failed", namespaceName)
			}
			break
		}
		...
		for {
			...
			if err != nil {
				...
				return errors.Wrapf(err, "[%s] failed to receive deployment ack", namespaceName)
			}
			if resp.Ack {
				// ack for the resource spec
				if !resp.GetSuccess() {
					return errors.Errorf("[%s] unable to deploy resource: %s %s", namespaceName, resp.GetResourceName(), resp.GetMessage())
				}
				deployCounter++
				if verbose {
...
func registerNamespace(deployTimeoutCtx context.Context, runtime pb.RuntimeServiceClient,
	...
	if err != nil {
		...
		return errors.Wrap(err, fmt.Sprintf("[%s] failed to update namespace configurations", namespaceName))
	} else if !registerResponse.Success {
		return fmt.Errorf("[%s] failed to update namespace configurations, %s", namespaceName, registerResponse.Message)
	}
