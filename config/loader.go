...
func LoadOptimusConfig() (*Optimus, error) {
	...
	if err = LoadConfig(optimus, fs, currPath, filepath.Dir(execPath), optimusDir); err != nil {
		return nil, errors.Wrapf(err, "error loading config")
	}

	// Load namespaces config
	if err = LoadNamespacesConfig(LoadConfig)(optimus.Namespaces, fs, currPath); err != nil {
		return nil, errors.Wrapf(err, "error loading namespaces config")
	}
