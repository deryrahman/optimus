...
func getSystemDefinedConfigs(job *scheduler.Job, interval interval.Interval, executedAt, scheduledAt time.Time) map[string]string {
	vars := map[string]string{
		...
		configExecutionTime: executedAt.Format(TimeSQLFormat), // TODO: it uses to support execution time sql format in maxcompute
	...
	if job.WindowConfig.GetVersion() == window.NewWindowVersion {
		...
		vars[configExecutionTime] = scheduledAt.Format(TimeSQLFormat) // TODO: it uses to support execution time sql format in maxcompute
